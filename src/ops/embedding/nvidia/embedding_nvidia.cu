...
template <typename T>
__global__ void embedding_kernel(T* out, const int64_t* index, const T* weight, ptrdiff_t out_s0, ptrdiff_t out_s1,
                                 ptrdiff_t weight_s0, ptrdiff_t weight_s1, int64_t weight_rows, size_t n, size_t d) {
    ...
    if (weight_row < 0 || weight_row >= weight_rows) {
        out[out_offset] = T{};
        return;
    }
...
template <typename T> void embedding_impl(tensor_t out, tensor_t index, tensor_t weight) {
    const size_t n = index->numel();
    const size_t d = weight->shape()[1];
    const int64_t weight_rows = static_cast<int64_t>(weight->shape()[0]);
    if (n == 0 || d == 0) {
        return;
    }
    ...
    embedding_kernel<T><<<dim3(grid_x, grid_y), block_size, 0, stream>>>(out_ptr, index_ptr, weight_ptr, out_s0, out_s1,
                                                                         weight_s0, weight_s1, weight_rows, n, d);
}
...
void embedding(tensor_t out, tensor_t index, tensor_t weight) {
    ...
    ASSERT(index->dtype() == LLAISYS_DTYPE_I64, "Embedding: Data Type of index must be LLAISYS_DTYPE_I64");
