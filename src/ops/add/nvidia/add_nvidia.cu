...
void add_(tensor_t c, tensor_t a, tensor_t b) {
    ...
    add_kernel<T><<<num_blocks, block_size>>>(c_ptr, a_ptr, b_ptr, c->numel());
}
...
void add_fp16(tensor_t c, tensor_t a, tensor_t b) {
    ...
    add_kernel_fp16<<<num_blocks, block_size>>>(c_ptr, a_ptr, b_ptr, c->numel());
}
...
void add_bf16(tensor_t c, tensor_t a, tensor_t b) {
    ...
    add_kernel_bf16<<<num_blocks, block_size>>>(c_ptr, a_ptr, b_ptr, c->numel());
}
void add_cublas_fp16(tensor_t c, tensor_t a,tensor_t b){
    ...
    api->memcpy_async(c_ptr,
                      b_ptr,
                      c->numel() * c->elementSize(),
                      LLAISYS_MEMCPY_D2D,
                      stream);

    status = cublasAxpyEx(handle,
                          n,
                          &alpha,
                          CUDA_R_32F,
                          a_ptr,
    ...
    // Make add deterministic for callers that read output immediately.
...
void add(tensor_t c, tensor_t a, tensor_t b) {
    ...
    case LLAISYS_DTYPE_F32:
        return add_cublas_fp32(c, a, b);
    case LLAISYS_DTYPE_BF16:
        return add_cublas_bf16(c, a, b);
    case LLAISYS_DTYPE_F16:
        return add_cublas_fp16(c, a, b);
    default:
